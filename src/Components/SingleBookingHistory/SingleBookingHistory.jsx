  ...
  return (
    <div className='reservation-card'>
      ...
      <div className='reservation-info'>
          ...
          <p>{t("SingleBooking.from")} <strong>{dateInit.slice(0, 10)}</strong> {t("SingleBooking.to")} <strong>{dateFinal.slice(0,10)}</strong> <span>({nights === 1 ? `${nights} ${t("SingleBooking.night")}` : `${nights} ${t("SingleBooking.nightS")}`})</span></p>
          <p>{t("SingleBooking.amount")} <span>${amount}</span></p>
          <button className='button' onClick={() => handleClick(bookingId)}>{t("SingleBooking.detailsBtn")}</button>
      </div>
    {!hotelInUserReviews.length
      ? (<div className="review-box-container">
          ...
          <button className="review-submit-button" onClick={handleSubmitReview}>{t("SingleBooking.submitBtn")}</button>
        </div>)
      : ""
    }
  </div>)
