...
const RankingChart = ({ startDate, endDate }) => {
  ...
  const handleDownloadExcel = async () => {
    swal({
      ...
      if (confirmed) {
        try {
          const response = await axios.get(`${import.meta.env.VITE_BACK_URL}/excel/rankings`, {
            params: {
        start_date: startDate.toISOString(),
        end_date: endDate.toISOString()
            },
          ...
          link.href = url;
          link.setAttribute('download', `Ranking of hotels from ${startDate} to ${endDate}.xlsx`);
