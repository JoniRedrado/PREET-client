...
const GestionRooms = () => {
  ...
  const [editableRoom, setEditableRoom] = useState(false);
  ...
  const getRooms = async () => {
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_BACK_URL}/rooms?type=${typeInput}&page=${currentPage}&size=${pageSize}`
      );
  ...
  const handleRoomToEdit = (id) => {
    dispatch(setRoomId(id));
    setEditableRoom(true);
  };

  const handleRoomEditBack = () => {
    setEditableRoom(false);
  };

  const renderContent = () => {
    if (editableRoom) {
      return (
        <div className="main-editable-room">
          <IoArrowBackCircle className="back-icon" onClick={handleRoomEditBack} />
          <UpdateRooms />
        </div>
      );
    } else if (showDeletedRooms && !editableRoom) {
      return (
        <div className="table-container2">
          <div className="header">
            <div className="search-users">
              ...
              <button onClick={handleSearch} className="button-users">
                {t("dashboard.search")}
              </button>
            </div>
            <div className="btn-deleted-container">
              <button onClick={handleShowDeletedRooms} type="button" className="button-users2">
                  ...
                  <tr key={rooms.id}>
                    ...
                    <td>
                      <i
                        ...
                        title="Update"
                        onClick={() => handleRoomToEdit(rooms.id)}
